 ...
 * @LastEditTime: 2024-08-12 23:06:03
...
int OLED_Init( )
{
    ...
    I2C_Write_Byte( 0xD5, IIC_CMD ); // set display clock divide ratio/oscillator frequency
...
{
    uint8_t i = 0;
    OLED_address( ( Line - 1 ) * 2, ( Column - 1 ) * 8 ); //一小行一小列
    for ( i = 0; i < 8; i++ )
        I2C_Write_Byte( 0x00, IIC_RAM );
    OLED_address( ( Line - 1 ) * 2 + 1, ( Column - 1 ) * 8 );
    for ( i = 0; i < 8; i++ )
        I2C_Write_Byte( 0x00, IIC_RAM );
}